...
df_adj %>%
  ggplot(aes(x = AGNO_FALLO, y = tasa, color = DISCIPLINA_OECD, group = DISCIPLINA_OECD)) +
  ...
  theme(
    axis.text.x  = element_text(angle = 45, hjust = 1),
    plot.caption = element_text(hjust = 0, size = 9, color = "gray40", face = "italic", margin = margin(t = 10))
  )
```
...
df_soc %>%
  ggplot(aes(x = factor(AGNO_FALLO), y = total, fill = GRUPO_ESTUDIO)) +
  geom_col(position = "stack") +
...
df_plot %>%
  ggplot(aes(x = AGNO_FALLO, y = monto_solic, color = GRUPO_ESTUDIO, group = GRUPO_ESTUDIO)) +
  geom_line(linewidth = 1) +
  geom_point(size = 2.5) +
...
df_plot %>%
  ggplot(aes(x = AGNO_FALLO, y = monto_prom, color = GRUPO_ESTUDIO, group = GRUPO_ESTUDIO)) +
  geom_line(linewidth = 1) +
  geom_point(size = 2.5) +
  scale_y_continuous(labels = scales::label_comma(prefix = "$")) +
  scale_x_continuous(breaks = unique(df_plot$AGNO_FALLO)) +
...
df_inst %>%
  ggplot(aes(x = AGNO_FALLO, y = n, color = INSTITUCION_PRINCIPAL, group = INSTITUCION_PRINCIPAL)) +
  geom_line(linewidth = 1) +
  geom_point(size = 2.5) +
...
df_macro %>%
  ggplot(aes(x = AGNO_FALLO, y = n, color = MACROZONA_MINCIENCIA, group = MACROZONA_MINCIENCIA)) +
  geom_line(linewidth = 1) +
  geom_point(size = 2.5) +
  scale_color_manual(values = colores_set2) +
